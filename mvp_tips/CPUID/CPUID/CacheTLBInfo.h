...
public:
    ...
    void Reset()
    {
        descriptors.SetSize(0);
    }
    void Add(UINT id)
    {
        descriptors.Add(TextDescriptor(id));
    }
    void Add(LPCTSTR L, LPCTSTR S)
    {
        descriptors.Add(TextDescriptor(L, S));
    }
protected:
    DECLARE_MESSAGE_MAP()
    ...
    public:
        ...
        TextDescriptor(LPCTSTR L, LPCTSTR S)
        {
            ID = 0;
            ShortDesc = S;
            LongDesc = L;
        }
        CString GetLongVersion()
        {
            CString s;
